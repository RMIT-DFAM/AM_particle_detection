...
-> The first is to use the reconstructed slice data as the data that is input.
-> The second is to use a matrix file (this is generated and saved by running the first startup option) with saved variable information
-> The third is to use a generated test data set (currently generated by Digital_Origin_Test_Case_Generation_For_Algorithm_v3.m)

If you just wish to generate the results as found in the afore mentioned publication, there are two options available to you. Please note that your
"Current Folder" for MATLAB, should be the folder that contains the reconstructed slice files, and the reconstruction log respectively.
...
These will either need to be updated manually in the codebase each time, or a slight adjustment to the code will be needed to implement them based on
user input. These can be found in the "%% Part 2 - externalMinorBounds (EMB) Preparation" section.
